...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if it's an AJAX request for username availability
    ...
    // Continue with the update if not an AJAX request
                    ...
                    <nav class="main-nav">
                        ...
                        <ul class="nav">
                            ...
                            if (!empty($_SESSION['LogedIn'])) {
                                ...
                                echo "<li><a class='active' href='UserPage.php'>{$username}</a></li><li>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <p>If you have any questions regarding how this page works, please contact us using the information below:</p>
                    <div class="row">
                        <div class="col-lg-12">
                ...
                <div class="col-lg-6">
                    <form id="contact-form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-lg-12">
                                <fieldset>
                                    <button type="button" id="change-submit" class="orange-button">Change</button>
                                </fieldset>
                            </div>
                        </div>
                    </form>
    ...
    <script>
        $(document).ready(function() {
            $('#change-submit').on('click', function(e) {
                e.preventDefault();
                var newUsername = $('#username_change').val();
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        username_change: newUsername,
                        ajax_check: '1'
                    },
                    success: function(response) {
                        var res = JSON.parse(response);
                        if (res.success) {
                            $('#contact-form').submit(); // Submit the form if username is available
                        } else {
                            alert(res.message); // Show alert if username is not available
                        }
                    }
                });
            });
